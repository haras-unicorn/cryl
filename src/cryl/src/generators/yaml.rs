...
mod tests {
  ...
  fn test_generate_yaml_from_json() {
    ...
    generate_yaml(&dest_path, Format::Json, &source_path, false).unwrap();
  ...
  fn test_generate_yaml_from_yaml() {
    ...
    generate_yaml(&dest_path, Format::Yaml, &source_path, false).unwrap();
  ...
  fn test_generate_yaml_from_toml() {
    ...
    generate_yaml(&dest_path, Format::Toml, &source_path, false).unwrap();
  ...
  fn test_generate_yaml_renew_true_overwrites() {
    ...
    generate_yaml(&dest_path, Format::Json, &source_path, true).unwrap();
  ...
  fn test_generate_yaml_source_not_found() {
    ...
    let result = generate_yaml(&dest_path, Format::Json, &source_path, false);
    assert!(result.is_err());
  }

  #[test]
  fn test_generate_yaml_subdirs() {
    let temp = TempDir::new().unwrap();
    let source_path = temp.path().join("subdir1").join("source.json");
    let dest_path = temp.path().join("subdir2").join("output.yaml");
